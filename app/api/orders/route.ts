...
// Vercel KV (Redis) – nur wenn KV_REST_API_URL gesetzt (Vercel-Deployment)
// Lokaler Fallback: JSON-Datei
const USE_KV = !!process.env.KV_REST_API_URL;
let kv: { lrange: (k:string,s:number,e:number)=>Promise<string[]>; rpush: (k:string,...v:string[])=>Promise<number> } | null = null;
if (USE_KV) {
  // dynamisch importieren damit lokaler Build nicht scheitert wenn Paket fehlt
  kv = await import("@vercel/kv").then(m => m.kv).catch(() => null);
}

// ─── Konfiguration (alle Werte über .env.local setzen) ───────────────────────
const ORDERS_FILE  = path.join(process.cwd(), "data", "orders.json");
const ADMIN_PIN    = process.env.ADMIN_PIN    ?? "rajmahal2024";   // ← bitte ändern!

// SMTP (z.B. Gmail, Strato, Ionos, eigener Server)
const SMTP_HOST    = process.env.SMTP_HOST    ?? "";
const SMTP_PORT    = Number(process.env.SMTP_PORT   ?? "587");
const SMTP_SECURE  = process.env.SMTP_SECURE  === "true";          // true = Port 465
const SMTP_USER    = process.env.SMTP_USER    ?? "";               // z.B. bestellungen@rajmahal.de
const SMTP_PASS    = process.env.SMTP_PASS    ?? "";               // App-Passwort
const MAIL_TO      = process.env.MAIL_TO      ?? "";               // Empfänger (Restaurant)
const MAIL_FROM    = process.env.MAIL_FROM    ?? SMTP_USER;        // Absender
...
async function readOrders(): Promise<Order[]> {
  if (USE_KV && kv) {
    try {
      const items = await kv.lrange("orders", 0, -1);
      return items.map((s) => JSON.parse(s) as Order);
    } catch { return []; }
  }
  try {
...
async function writeOrders(orders: Order[]): Promise<void> {
  if (USE_KV && kv) return; // KV nutzt rpush statt vollständiges Schreiben
...
<style>
  ...
  @media print{
    ...
    .page{max-width:100%;border:none;border-radius:0}
...
<div class="page">
  ...
  <div class="hd">
    <div>
      <h1>🍛 RAJMAHAL</h1>
      <div class="sub">Indisches Restaurant · Friedrich-Ebert-Str. 16 · 04435 Schkeuditz<br>
        Tel. 034204–360529 · 0176–64 91 88 23</div>
    </div>
    <div class="nr">
      <strong>#${order.id}</strong>
      ${forCustomer ? "Ihre Bestellung" : "Bestelleingang"}
    </div>
  </div>
  ...
  <div class="meta">
    <table>
      ...
      <tr><td>Zahlung</td><td>${isCash
        ? (isDelivery ? "💵 Bar bei Lieferung" : "💵 Bar bei Abholung")
        : `💳 PayPal (${order.paypalOrderId ?? ""})`}</td></tr>
...
<!-- Druckbutton nur im Browser, nicht beim Drucken -->
...
async function sendOrderMail(order: Order): Promise<void> {
  ...
  // ─ An Kunde (wenn E-Mail angegeben) ─
  if (order.customer.email) {
    await transporter.sendMail({
      ...
      text: `Ihre Bestellung #${order.id} wurde erfolgreich aufgegeben. Gesamtbetrag: ${fmt(order.total)}`,
    });
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    const orders = await readOrders();
    order.id = await nextOrderId(orders);  // fortlaufende Nummer
    await appendOrder(order);

    // E-Mail asynchron senden (blockiert nicht die Antwort an den Kunden)
